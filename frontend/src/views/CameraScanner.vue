  ...
  <div class="lumber-scanner-layout">
    ...
    <div class="scanner-main">
      ...
      <div id="realtimeSection" class="section-content" v-show="currentMode === 'realtime'">
        ...
        <div class="debug-panel">
          ...
          <div class="hsv-controls">
            ...
            <label>Hmax <input type="range" id="hmax" min="0" max="179" value="25"></label>
            <label>Smin <input type="range" id="smin" min="0" max="255" value="40"></label>
            <label>Smax <input type="range" id="smax" min="0" max="255" value="255"></label>
            <label>Vmin <input type="range" id="vmin" min="0" max="255" value="50"></label>
      ...
      <div class="mobile-instructions">
        <h3>📱 Tips for Accurate Scanning</h3>
        <ul>
          <li>✅ <strong>Calibration is critical</strong> - Use a ruler or measuring tape for accurate results</li>
          <li>✅ <strong>Good lighting</strong> - Scan in daylight or well-lit areas for better color detection</li>
          <li>✅ <strong>Hold steady</strong> - Keep camera still and perpendicular to the tree trunk</li>
          <li>✅ <strong>Full tree view</strong> - Include the full diameter and height of the trunk</li>
          <li>✅ <strong>Background matters</strong> - Plain background helps detection accuracy</li>
          <li>✅ <strong>Use the mask preview</strong> - Enable "Show Mask Preview" debug option to fine-tune HSV values</li>
          <li>✅ <strong>Alt+Click on camera</strong> - Sample a pixel to auto-calibrate HSV values</li>
        </ul>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    initializeScannerApp() {
      ...
      try {
        window.scannerState = {
          ...
          // Enhanced HSV tuning defaults optimized for COCOLUMBER bark detection
          // Brown range: H(0-25) + Red wrap(150-180), higher saturation, avoid shadows
          hsv: { hmin: 0, hmax: 25, smin: 40, smax: 255, vmin: 50, vmax: 255 },
          showMask: false,
        ...
        window.MIN_CONTOUR_AREA = 3000  // Increased for better filtering
    ...
    processFrame() {
      if (!window.scannerStream) {
        console.warn('❌ Scanner not ready: stream not available')
        return
      }
      ...
      try {
        ...
        // Convert canvas frame to base64 and send to backend for detection
        const frameImageData = canvasOutput.toDataURL('image/jpeg', 0.8)
        this.sendToBackendDetection(frameImageData, 'realtime')
      } catch (err) {
    ...
    enhancedTreeDetection(src) {
      ...
      try {
        ...
        // Enhanced HSV thresholds optimized for COCOLUMBER (brown/reddish bark)
        // Default: hue 0-25 (browns) + 150-180 (reds wrapping around)
        const hsvRange = (window.scannerState && window.scannerState.hsv) ? window.scannerState.hsv : { 
          hmin: 0,      // Lower brown hue
          hmax: 25,     // Upper brown hue (tighter brown range)
          smin: 40,     // Higher saturation for better bark detection
          smax: 255,    // Full saturation
          vmin: 50,     // Higher value threshold to avoid shadows
          vmax: 255     // Full brightness
        }
        ...
        // For browns/reds that wrap around hue (150-180)
        let low2 = new cv.Mat(hsv.rows, hsv.cols, hsv.type(), [150, hsvRange.smin, hsvRange.vmin])
        ...
        // Enhanced morphology for better noise reduction and contour definition
        let kernel_open = cv.getStructuringElement(cv.MORPH_ELLIPSE, new cv.Size(5, 5))
        let kernel_close = cv.getStructuringElement(cv.MORPH_ELLIPSE, new cv.Size(11, 11))
        ...
        let kernel_dilate = cv.getStructuringElement(cv.MORPH_ELLIPSE, new cv.Size(3, 3))
        cv.dilate(mask, mask, kernel_dilate, new cv.Point(-1, -1), 2)
        ...
        if (contours.size() > 0) {
          ...
          if (maxContourIndex !== -1) {
            ...
            // Use fitted ellipse for more accurate circular measurements
            let rect = cv.boundingRect(trunk)
            let moments = cv.moments(trunk)
            let contourArea = moments.m00
            
            // Calculate circularity (how round the object is)
            let perimeter = cv.arcLength(trunk, true)
            let circularity = (4 * Math.PI * contourArea) / (perimeter * perimeter)
            
            // Better diameter calculation using average of width and height
            let diameter_px = Math.sqrt((rect.width * rect.width + rect.height * rect.height) / 2)
            let diameter_cm = diameter_px * window.scannerState.pixelToCm
            
            let height_cm = rect.height * window.scannerState.pixelToCm
            let radius_cm = diameter_cm / 2
            
            // More accurate volume calculation with shape factor
            // Adjusted for actual tree cross-section (not perfect cylinder)
            let shapeFactorGrade = circularity > 0.75 ? 0.9 : (circularity > 0.6 ? 0.8 : 0.7)
            let volume_cm3 = Math.PI * Math.pow(radius_cm, 2) * height_cm * shapeFactorGrade
            
            let weight_kg = (volume_cm3 * window.WOOD_DENSITY_G_CM3) / 1000
            let num_lumber = Math.max(0, (volume_cm3 / window.LUMBER_VOLUME_CM3) * 0.75)

            // Improved quality grading based on multiple factors
            let quality = 'Low'
            if (contourArea > 15000 && circularity > 0.7 && diameter_cm > 10) {
              quality = 'High'
            } else if (contourArea > 8000 && circularity > 0.6 && diameter_cm > 7) {
              quality = 'Medium'
            }
    ...
    startCameraCalibration() {
      ...
      if (!refWidth || refWidth <= 0) {
        alert("⚠️ Please enter a valid reference width in centimeters\n\nTips:\n- Use a ruler or measuring tape\n- Use 10cm for standard calibration\n- Hold object perpendicular to camera")
        return
      }
      ...
      if (calibrationStatus) {
        calibrationStatus.textContent = "👆 Tap two points on your reference object (ruler/edge)"
        calibrationStatus.style.color = "#e74c3c"
        calibrationStatus.style.display = "block"
      }

      this.updateCameraFeedback("📐 Calibration: Click two points on edge of object")
    ...
    analyzeUploadedImage() {
      ...
      this.sendToBackendDetection(imageData, 'upload')
    },
    async sendToBackendDetection(imageData, source = 'upload') {
      try {
        ...
        // Check if brown/tree was actually detected
        if (!result.detectedClass || (result.detectedClass !== 'cocolumber' && result.detectedClass !== 'wood' && result.detectedClass !== 'tree')) {
          document.getElementById('detectionInfo').textContent = '❌ No brown object detected - try another image'
          document.getElementById('status').textContent = 'Status: No detection'
          this.showNoDetection()
          return
        }

        // Display results
        document.getElementById('detectionInfo').textContent = `✅ Detected: ${result.detectedClass || 'Unknown'} (Confidence: ${result.confidence || '0'}%)`
        document.getElementById('status').textContent = 'Status: Detection complete'

        if (result.height && result.width && result.diameter) {
          document.getElementById('diameterResult').textContent = `${result.diameter || '--'} cm`
          document.getElementById('heightResult').textContent = `${result.height || '--'} m`
          document.getElementById('volumeResult').textContent = `${result.volume || '--'} m³`
          document.getElementById('weightResult').textContent = `${result.weight || '--'} kg`
          document.getElementById('lumberResult').textContent = `${result.estimatedLumber || '--'} bf`
          document.getElementById('qualityResult').textContent = result.quality || 'N/A'
        }

        console.log('✅ Backend detection result:', result)
      } catch (error) {
        console.error('❌ Detection error:', error)
        document.getElementById('detectionInfo').textContent = `❌ Detection error: ${error.message}`
        document.getElementById('status').textContent = 'Status: Detection error'
        this.showNoDetection()
      }
    },
    ...
    updateEnhancedResults(diameter, height, volume, weight, lumber) {
      ...
      // Enhanced precision display for better accuracy feedback
      diameterResult.textContent = `${diameter.toFixed(2)} cm`
      heightResult.textContent = `${height.toFixed(2)} cm`
      volumeResult.textContent = `${volume.toFixed(1)} cm³`
      weightResult.textContent = `${weight.toFixed(2)} kg`
      lumberResult.textContent = `${Math.round(lumber * 10) / 10} pcs`
